...
- Encode invariants in types: hard-to-misuse types > runtime checks > prose comments, per `pstack-principle-encode-lessons-in-structure`.
...
- Short call chains. If tracing the flow needs more than three files, flatten the hierarchy, per `pstack-principle-laziness-protocol`.
